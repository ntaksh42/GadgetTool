# Azure DevOps REST API を使用してダミーのワークアイテムを作成するスクリプト（修正版）
...
Write-Host "Creating 45 dummy bug tickets in Azure DevOps $Project..." -ForegroundColor Green
...
for ($i = 1; $i -le 45; $i++) {
    try {
        ...
        Write-Host "[$i/45] Creating: $title (Priority:$priority)" -ForegroundColor Cyan
